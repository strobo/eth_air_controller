...
<script>\
state={a:'off',b:30};state.send=function(){$.get('/ir',{power:state.a,temp:state.b},function(a){console.log(a)})};$(function(){$('#power').text(state.a==='off'?'ON':'OFF').toggle(function(){$(this).text('OFF').toggleClass('primary',!1);state.a='on';state.b=$('#selectTemp').val()-0;state.send()},function(){$(this).text('ON').toggleClass('primary',!0);state.a='off';state.b=$('#selectTemp').val()-0;state.send()});$('#selectTemp').change(function(){state.b=$('#selectTemp').val()-0;state.a==='on'&&state.send()})});\
</script>\
</head><body>\
<div class='container'>\
<h1>RemoteAirController</h1>\
<p><select id='selectTemp'name='temp'style='font-size:27px;width:100px;height:54px'>\
